...
<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedUser=item.userName">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="delAddressConfirm(item.userName)">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item.userName)"><i>设为默认</i></a>
                    </div>
                    <div class="addr-opration addr-default" v-if="item.isDefault">默认地址</div>
                  </li>
                  <li class="addr-new">
                    <div class="add-new-inner">
                      <i class="icon-add"  @click="addNewaddressModalFlag=true">
                        <svg class="icon icon-add"><use xlink:href="#icon-add"></use></svg>
                      </i>
                      <p   @click="addNewaddressModalFlag=true" style="color:#d1434a;">新增地址</p>
                    </div>
                  </li>
                </ul>
              </div>
            ...
            <div class="shipping-method-wrap">
              <div class="shipping-method">
                <ul>
                  <li class="check">
                    <div class="name">顺丰包邮哦～亲</div>
                    <div class="price">免费</div>
                    <div class="shipping-tips">
                      <p>大约两天后您能收到此件快递哦～亲</p>
                    </div>
                  </li>
                </ul>
              </div>
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red"
                           v-bind:to="{path:'orderConfirm',query:{'userName':selectedUser}}">下一步</router-link>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 新增地址模态框 -->
      <div class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':addNewaddressModalFlag}">
          <div class="md-modal-inner">
            <div class="md-top">
              <div class="md-title">新增地址</div>
              <button class="md-close" @click="addNewaddressModalFlag=false">Close</button>
            </div>

            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap">
                  <span class="error error-show" v-show="errorTip">用户名或者地址或手机号不能为空</span>
                </div>
                <ul>
                  <li class="regi_form_input">
                    <i class="icon IconPeople"></i>
                    <input type="text" tabindex="1" name="userName" v-model="userName" class="regi_login_input regi_login_input_left" placeholder="收货人姓名" data-type="loginname">
                  </li>
                  <li class="regi_form_input noMargin">
                    <input type="text" tabindex="2"  name="streetName" v-model="streetName" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="收货地址" @keyup.enter="addNewaddress">
                  </li>
                  <li class="regi_form_input noMargin">
                    <input type="text" tabindex="3"  name="tel" v-model="tel" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="手机号码" @keyup.enter="addNewaddress">
                  </li>
                </ul>
              </div>
              <div class="login-wrap">
                <a href="javascript:;" class="btn-login" @click="addNewaddress">确认添加</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default{
      ...
      methods:{
          init(){
             ...
             console.log(this.good);
              axios.get("/users/addressList").then((response)=>{
                  ...
                  this.selectedUser = this.addressList[this.checkIndex].userName;
              });
          },
          // 展开与收起
          expand(){
          ...
          addNewaddress(){
            console.log('11111');
            if(!this.userName || !this.streetName || !this.tel){
                  this.errorTip = true;
                  return;
                }
             var param = {
                ...
                axios.post("/users/addNewaddress",{
                  ...
                  console.log(res);
                  this.addNewaddressModalFlag = false;
